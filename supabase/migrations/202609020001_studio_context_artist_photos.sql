alter table artist_profiles
  add column if not exists studio_photo_paths jsonb not null default '{}'::jsonb;
  ...
  return jsonb_build_object(
    ...
    case
      when v_artist_profile.id is null then null
        ...
        'studio_photo_paths', coalesce(v_artist_profile.studio_photo_paths, '{}'::jsonb),
...
begin
  ...
  insert into artist_profiles (
    artist_id, artistic_name, bio, specialties, primary_specialty, years_experience,
    payment_methods, whatsapp, instagram, facebook, tiktok, website, photo_path,
    studio_photo_paths, portfolio_paths, use_studio_location, address_line, city,
    state, postal_code, latitude, longitude, address_references, google_maps_url, updated_at
  )
  values (
    v_artist.id,
    ...
    coalesce(p_profile -> 'studio_photo_paths', '{}'::jsonb),
    ...
    nullif(trim(coalesce(p_profile ->> 'postal_code', '')), '')::text,
  ...
  set
    ...
    studio_photo_paths = excluded.studio_photo_paths,
    ...
    updated_at = now()
  returning * into v_artist_profile;

  update artists
  set display_name = v_artist_profile.artistic_name, updated_at = now()
  where id = v_artist.id
  returning * into v_artist;

  return public.studio_flow_artist_get_own_profile(v_artist.id);
end;
$$;
...
begin
  ...
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        ...
        'studioPhotoUrl', coalesce(ap.studio_photo_paths ->> v_studio_id::text, ap.photo_path),
        'studio_photo_url', coalesce(ap.studio_photo_paths ->> v_studio_id::text, ap.photo_path),
        'role', asm.role,
        'status', asm.status,
  ...
  select coalesce(jsonb_agg(invitation_payload), '[]'::jsonb)
  into v_invitations
  from (
    select jsonb_build_object(
      ...
      'artistName', coalesce(ap.artistic_name, a.display_name, p.display_name, invitation.invited_email),
      'artist_name', coalesce(ap.artistic_name, a.display_name, p.display_name, invitation.invited_email),
      'status', invitation.status,
      'createdAt', invitation.created_at,
      'created_at', invitation.created_at,
      'expiresAt', invitation.expires_at,
      'expires_at', invitation.expires_at
    ) as invitation_payload
    from studio_artist_invitations invitation
