...
begin
  ...
  select jsonb_build_object(
    ...
    'alreadyMember', asm.id is not null and asm.archived_at is null and asm.status = 'active',
    'already_member', asm.id is not null and asm.archived_at is null and asm.status = 'active'
  )
  into v_artist
  from profiles p
...
begin
  ...
  where (
      (service.owner_type = 'membership' and service.membership_id = v_membership.id)
      or (service.owner_type = 'studio' and service.studio_id = v_studio_id)
    )
