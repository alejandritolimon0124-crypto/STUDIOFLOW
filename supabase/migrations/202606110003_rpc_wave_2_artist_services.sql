create or replace function public.studio_flow_artist_current_owned_artist(
...
create or replace function public.studio_flow_artist_get_or_create_service_tier(
  p_tier_code text
)
returns service_tiers
language plpgsql
security definer
set search_path = public
as $$
declare
  ...
  v_code service_tier_code;
begin
  v_code := case
    when lower(coalesce(p_tier_code, 'basic')) in ('basic', 'medium', 'premium', 'vip')
      then lower(coalesce(p_tier_code, 'basic'))::service_tier_code
    else 'basic'::service_tier_code
  end;

  insert into service_tiers (code, label, default_points, status)
  values (
    v_code,
    case v_code
...
create or replace function public.studio_flow_artist_get_service_offerings(
  p_artist_id uuid default null,
...
begin
  ...
  select coalesce(
    jsonb_agg(studio_flow_artist_service_to_json(so.id) order by so.created_at desc),
    '[]'::jsonb
  )
  into v_services
  from service_offerings so
  where so.owner_type = 'artist'
    and so.artist_id = v_artist.id
...
begin
  ...
  values (auth.uid(), 'marketplace', 'service_offerings', v_service.id, v_artist.id, 'artist_service_activated', to_jsonb(v_current), to_jsonb(v_service));
...
begin
  ...
  values (auth.uid(), 'marketplace', 'service_offerings', v_service.id, v_artist.id, 'artist_service_suspended', to_jsonb(v_current), to_jsonb(v_service), jsonb_build_object('reason', p_reason));
...
begin
  ...
  insert into audit_events (actor_profile_id, context, entity_type, entity_id, artist_id, event_type, before_data, after_data, metadata)
  values (auth.uid(), 'marketplace', 'service_offerings', v_service.id, v_artist.id, 'artist_service_archived', to_jsonb(v_current), to_jsonb(v_service), jsonb_build_object('reason', p_reason));

  return jsonb_build_object('service', studio_flow_artist_service_to_json(v_service.id));
end;
$$;

revoke all on function public.studio_flow_artist_current_owned_artist(uuid) from public;
...
revoke all on function public.studio_flow_artist_get_or_create_service_tier(text) from public;
revoke all on function public.studio_flow_artist_get_service_offerings(uuid, boolean) from public;
revoke all on function public.studio_flow_artist_create_service_offering(jsonb, uuid) from public;
revoke all on function public.studio_flow_artist_update_service_offering(uuid, jsonb) from public;
revoke all on function public.studio_flow_artist_activate_service_offering(uuid) from public;
revoke all on function public.studio_flow_artist_suspend_service_offering(uuid, text) from public;
revoke all on function public.studio_flow_artist_archive_service_offering(uuid, text) from public;

grant execute on function public.studio_flow_artist_get_service_offerings(uuid, boolean) to authenticated;
grant execute on function public.studio_flow_artist_create_service_offering(jsonb, uuid) to authenticated;
grant execute on function public.studio_flow_artist_update_service_offering(uuid, jsonb) to authenticated;
grant execute on function public.studio_flow_artist_activate_service_offering(uuid) to authenticated;
grant execute on function public.studio_flow_artist_suspend_service_offering(uuid, text) to authenticated;
grant execute on function public.studio_flow_artist_archive_service_offering(uuid, text) to authenticated;
