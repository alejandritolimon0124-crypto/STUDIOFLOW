...
begin
  ...
  with appointment_amounts as (
    select
      ...
      case
        when appt.status in ('cancelled', 'no_show') then 0
        when comm.status = 'void' then 0
        else coalesce(comm.amount, ae.platform_fee_amount, round(coalesce(so.price_amount, 0) * 0.10, 2), 0)
    ...
    where appt.status not in ('cancelled', 'no_show')
  )
  select
