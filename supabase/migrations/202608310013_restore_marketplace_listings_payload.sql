...
begin
  ...
  enriched_listings as (
    select
      ...
      coalesce(active_promotions.promotions, '[]'::jsonb) as active_promotions,
      coalesce(flow_rewards.rewards, '[]'::jsonb) as rewards
        ...
        coalesce(
          jsonb_agg(
            jsonb_build_object(
              ...
              'owner_type', service_rows.owner_type,
              'priceAmount', service_rows.price_amount,
              'price_amount', service_rows.price_amount,
              'durationMinutes', service_rows.duration_minutes,
              'duration_minutes', service_rows.duration_minutes,
              ...
              'service_tier', service_rows.service_tier,
    ...
    left join lateral (
      select coalesce(jsonb_agg(jsonb_build_object(
        ...
        'status', p.status,
        'rules', p.rules
      ) order by p.updated_at desc), '[]'::jsonb) as promotions
      from promotions p
    ...
    left join lateral (
      select coalesce(jsonb_agg(jsonb_build_object(
        ...
        'status', r.status
      ) order by r.points_cost), '[]'::jsonb) as rewards
      from rewards r
      where r.status = 'active'
        and r.archived_at is null
        and (
          (r.scope_type = 'artist' and r.artist_id = lt.artist_id)
          or (r.scope_type = 'studio' and r.studio_id = lt.studio_id)
        )
        and (
          exists (
            select 1
            from artist_marketing_preferences amp
            where amp.artist_id = lt.artist_id
              and amp.flow_points_enabled
          )
          or exists (
            select 1
            from promotions flow_promo
            where flow_promo.scope_type = r.scope_type::text::promotion_scope_type
  ...
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        ...
        'artist_name', coalesce(artistic_name, artist_display_name),
        'studioName', coalesce(commercial_name, studio_name),
        'studio_name', coalesce(commercial_name, studio_name),
        ...
        'profile', jsonb_build_object(
          ...
          'photo_path', case when profile_type = 'studio' then logo_path else photo_path end,
          ...
          'bio', case when profile_type = 'studio' then studio_description else artist_bio end,
          ...
          'contact_links', jsonb_build_object(
            'whatsapp', whatsapp,
            'instagram', instagram,
            'facebook', facebook,
            'tiktok', tiktok,
          ...
          'professionalLocation', jsonb_build_object(
            ...
            'address_line', case when profile_type = 'studio' then studio_address_line else artist_address_line end,
          ...
          'professional_location', jsonb_build_object(
            'useStudioLocation', use_studio_location,
            'use_studio_location', use_studio_location,
            'addressLine', case when profile_type = 'studio' then studio_address_line else artist_address_line end,
            'address_line', case when profile_type = 'studio' then studio_address_line else artist_address_line end,
        ...
        'studio', case when studio_id is null then null else jsonb_build_object(
          ...
          'profile', jsonb_build_object(
            ...
            'commercial_name', coalesce(commercial_name, studio_name),
            'description', studio_description,
            'addressLine', studio_address_line,
            'address_line', studio_address_line,
            'city', studio_city,
