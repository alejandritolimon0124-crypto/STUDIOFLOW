...
begin
  ...
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        ...
        'studioPhotoUrl', nullif(ap.studio_photo_paths ->> v_studio_id::text, ''),
        'studio_photo_url', nullif(ap.studio_photo_paths ->> v_studio_id::text, ''),
        'role', asm.role,
        'status', asm.status,
        ...
        'active', asm.status in ('active', 'accepted', 'approved', 'activo', 'aceptada', 'aprobada') and asm.archived_at is null
      )
  ...
  where asm.studio_id = v_studio_id
    ...
    and asm.status not in ('inactive', 'archived', 'rejected', 'inactivo', 'archivado', 'rechazado');
...
begin
  ...
  select jsonb_build_object(
    ...
    'alreadyMember', asm.id is not null and asm.archived_at is null,
    'already_member', asm.id is not null and asm.archived_at is null
  )
  into v_artist
  from profiles p
...
begin
  ...
  where id = p_membership_id
    ...
    and status in ('active', 'accepted', 'approved', 'activo', 'aceptada', 'aprobada')
